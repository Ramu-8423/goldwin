...
{
    public function bet_pdf(? string $id,? string $user_id){
        ...
        $bet_data = DB::table('bets')->where('id',$id)->where('user_id',$user_id)->first();
       // dd($bet_data);
    ...
    public function status_pdf(? string $id,? string $status,? string $user_id){
        
           $validator = Validator::make(['id'=>$id,'status'=>$status,'user_id'=>$user_id],[
            'id'=>'required|exists:bets,id',
            'user_id'=>'required|exists:bets,user_id',
            'status'=>'required'
            ]);
         ...
         if($validator->fails()){
          ...
          $bet_data = DB::table('bets')->where('id',$id)->where('user_id',$user_id)->where('status',$status)->first();
        if(!$bet_data){
