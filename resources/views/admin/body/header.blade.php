 <div id="content">
               <!-- topbar -->
               <div class="topbar">
                  <nav class="navbar navbar-expand-lg navbar-light">
                     <div class="full">
                        <button type="button" id="sidebarCollapse" class="sidebar_toggle"><i class="fa fa-bars"></i></button>
                        <div class="logo_section">
                             <?php
                              $authid = Session::get('Auth_id');
                              use Illuminate\Support\Facades\DB;
                              $wallet = DB::table('admins')->select('wallet')->where('id',  $authid)->first();
                              $role = Session::get('role_id');
                             ?>
                           <a href=""><h1 class="mt-3" style="color:white;font-weight:25px;margin-left:3px;"><span style="color:#ff5722;">Gold Win</span>
                           <?php
                              if($role!=1)
                              {
                                 echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Wallet ₹ : &nbsp;" . htmlspecialchars($wallet->wallet);
                              }
                           
                           ?>
                           </h1></a>
                        </div>
                        <div class="right_topbar">
                           <div class="icon_info">
                              <!--<ul>-->
                              <!--   <li><a href="#"><i class="fa fa-bell-o"></i><span class="badge">2</span></a></li>-->
                              <!--   <li><a href="#"><i class="fa fa-question-circle"></i></a></li>-->
                              <!--   <li><a href="#"><i class="fa fa-envelope-o"></i><span class="badge">3</span></a></li>-->
                              <!--</ul>-->
                              <ul class="user_profile_dd">
                                 <li>
                              
                                    <a class="dropdown-toggle" data-toggle="dropdown"><img class="img-responsive rounded-circle" src="images/layout_img/user_img.jpg" alt="#" /><span class="name_user"> {{ Session::get('terminal_id') }}</span></a>
                                    <div class="dropdown-menu">
                                       <a class="dropdown-item" href="profile.html">My Profile</a>
                                       <a class="dropdown-item" href="{{route('logout')}}"><span>Log Out</span> <i class="fa fa-sign-out"></i></a>
                                    </div>
                                 </li>
                              </ul>
                           </div>
                        </div>
                     </div>
                  </nav>
               </div>